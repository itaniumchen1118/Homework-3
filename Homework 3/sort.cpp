...
struct Sensor {
  ...
  Sensor(IdType i) : id(i) {
    ...
    for (size_t k = 0; k < NREADINGS; k++) readings[k] = rand() % 100;
...
inline bool compareSensor(const Sensor& lhs, const Sensor& rhs) {
  ...
  if (lhs.avg > rhs.avg) return true;
...
bool isSorted(const vector<Sensor>& s) {
  ...
  for (size_t k = 1; k < s.size(); k++) {
    if (compareSensor(s[k], s[k - 1])) return false;
  }
  return true;
}
...
int main() {
  ...
  vector<IdType> ids;
  for (size_t j = 0; j < nsensors; j++) ids.push_back(IdType(j));
  // The following are three options:
  // OPTION 1: good for C++ 14 or earlier - if using Visual Studio Community or
  // Enterprise, this should work random_shuffle(ids.begin(),ids.end());  // from <algorithm>
  ...
  */
  ...
  // destruction time.
  {
    ...
    sort(spointers.begin(), spointers.end(), compareSensorPtr);
    ...
    for (int i = 0; i < spointers.size(); i++) {
